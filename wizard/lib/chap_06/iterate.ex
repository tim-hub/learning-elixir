defmodule Wizard.Iterate do
  ...
  def example2() do
    ...
    keys = for {k,v} <- symbols, do: k
    IO.inspect(keys, label: "List the keys")

    values = for {_,v} <- symbols, do: v
  ...
  def example3 do
    ...
    joins = for a <- amounts, s <- sizes, i <- items do "#{a} #{s} of #{i}" end
  ...
  def example5 do
    ...
    iterate = Enum.map(elements, fn(e) -> Map.get(e, :symbol) end)
