defmodule Wizard.FlowChallenge do
  ...
  def if_control() do
    Enum.reduce(
      elements(),
      [],
      fn %{name: name, group: group} = e, acc ->
        classification =
  ...
  def cond_control() do
    Enum.reduce(
      ...
      fn %{name: name, group: group} = e, acc ->
        classification =
          cond do
            group === 14 or name === "Hydrogen" -> "other nonmetal"
    ...
    Enum.reduce(
      ...
      fn e, acc ->
        classification = case e do
          %{name: ele, group: g} when ele === "Hydrogen" or g === 14 -> "other nonmetal"
          %{group: g} when g === 1 -> "alkali metal"
          %{group: g} when g === 9 -> "transition metal"
          _ -> "Unknown"
        end
        [Map.put(e, :classification, classification) | acc]
      end
